...
export class Game {
  private _lastSymbol = SYMBOLS.EMPTY;
  ...
  private validateFirstMove(player: string) {
    if (this._lastSymbol == SYMBOLS.EMPTY) {
      if (player == SYMBOLS.O) {
  ...
  private validatePositionIsEmpty(x: number, y: number) {
    if (this._board.TileAt(x, y).Symbol != SYMBOLS.EMPTY) {
  ...
  private isFirstRowFull() {
    return (
      this._board.TileAt(0, 0)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(0, 1)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(0, 2)!.Symbol != SYMBOLS.EMPTY
    );
  }
  ...
  private isSecondRowFull() {
    return (
      this._board.TileAt(1, 0)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(1, 1)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(1, 2)!.Symbol != SYMBOLS.EMPTY
    );
  }
  ...
  private isThirdRowFull() {
    return (
      this._board.TileAt(2, 0)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(2, 1)!.Symbol != SYMBOLS.EMPTY &&
      this._board.TileAt(2, 2)!.Symbol != SYMBOLS.EMPTY
    );
  }
...
class Board {
  ...
  constructor() {
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        const tile: Tile = { X: i, Y: j, Symbol: SYMBOLS.EMPTY };
